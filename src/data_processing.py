...
def main(input_path, output_path):
    ...
    for region in REGIONS:
        ...
        for index, row in group.iterrows():
            ...
            if row["energy"] == "gen":
                df_clean.rename(columns={"quantity": f"quantity_{row['source']}"}, inplace=True)

            region_dfs.append(df_clean)
        
        region_df = reduce(lambda left, right: pd.merge(left, right, left_index=True, right_index=True, how='outer'), region_dfs)
        cols_to_add = list(filter(lambda col: col.startswith("quantity"), region_df.columns))
